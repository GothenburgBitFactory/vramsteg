...
# Environment flags to control skipping of vramsteg tests
VRAMSTEG_SKIP = os.environ.get("VRAMSTEG_SKIP", False)
# Environment flags to control use of PATH or in-tree binaries
VRAMSTEG_USE_PATH = os.environ.get("VRAMSTEG_USE_PATH", False)

UUID_REGEXP = ("[0-9A-Fa-f]{8}-" + ("[0-9A-Fa-f]{4}-" * 3) + "[0-9A-Fa-f]{12}")


def vramsteg_binary_location(cmd="vramsteg"):
    """ ../src/ is used by default.
    """
    return os.path.join(BIN_PREFIX, cmd)
    return binary_location(cmd, VRAMSTEG_USE_PATH)
